...
<div id="htmlcontent_{$hook|escape:'htmlall':'UTF-8'}"{if $hook == 'footer'} class="footer-block col-xs-12 col-sm-12"{/if}>
	<ul class="htmlcontent-home clearfix row">
		{foreach name=items from=$htmlitems item=hItem}
			...
			{else}
				<li class="htmlcontent-item-{$smarty.foreach.items.iteration|escape:'htmlall':'UTF-8'} col-xs-12 col-sm-6 col-md-3">
			{/if}
					<div class="row">
						{if $hItem.url}
						...
						{/if}
							...
							{if $hItem.title && $hItem.title_use == 1}
								<h3 class="item-title"><span>{$hItem.title|escape:'htmlall':'UTF-8'}</span> <i class="item-icon"></i></h3>
							{/if}
							{if $hItem.html}
								<div class="item-html">
