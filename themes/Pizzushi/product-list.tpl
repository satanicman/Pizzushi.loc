...
{if isset($products) && $products}
	...
	{foreach from=$products item=product name=products}
		...
		{if $smarty.foreach.products.iteration%$nbItemsPerLine == 1} 
			<li class="product_list_line">
				<ul{if isset($id) && $id} id="{$id}"{/if} class="product_list grid clearfix{if isset($class) && $class} {$class}{/if}">
				{assign var='openet' value=1}
		{/if}
		<li class="ajax_block_product{if $page_name == 'index' || $page_name == 'product'} col-xs-12 col-sm-4 col-md-3{else} col-xs-12 col-sm-6 col-md-3{/if}{if $smarty.foreach.products.iteration%$nbItemsPerLine == 0} last-in-line{elseif $smarty.foreach.products.iteration%$nbItemsPerLine == 1} first-in-line{/if}{if $smarty.foreach.products.iteration > ($smarty.foreach.products.total - $totModulo)} last-line{/if}{if $smarty.foreach.products.iteration%$nbItemsPerLineTablet == 0} last-item-of-tablet-line{elseif $smarty.foreach.products.iteration%$nbItemsPerLineTablet == 1} first-item-of-tablet-line{/if}{if $smarty.foreach.products.iteration%$nbItemsPerLineMobile == 0} last-item-of-mobile-line{elseif $smarty.foreach.products.iteration%$nbItemsPerLineMobile == 1} first-item-of-mobile-line{/if}{if $smarty.foreach.products.iteration > ($smarty.foreach.products.total - $totModuloMobile)} last-mobile-line{/if}">
			...
			<div class="product-container" itemscope itemtype="https://schema.org/Product" id="product_container_{$product.id_product}">
				<div class="left-block">
					<a class="quick-view product-name" href="{$product.link|escape:'html':'UTF-8'}" rel="{$product.link|escape:'html':'UTF-8'}">
						<div class="product-image-container" ontouchstart="this.classList.toggle('hover');">
							<span class="product_img_link" id="product_img_{$product.id_product}">
					...
					<div class="button-container">
						{foreach from=$combinations key=k item=comb}
						    {if $k == $product.id_product}
						        {foreach from=$comb item=attr name=attr}
											<div class="label_wrap">
												<input type="radio" name="label_radio_{$attr.id_product}" class="not_unifrom hidden label_radio" id="label_{$attr.id_product_attribute}"{if !$priceDisplay} data-price="{convertPrice price=$product.price + $attr.price}"{/if} data-id-product-attribute="{$attr.id_product_attribute}" {if $smarty.foreach.attr.index == 0}checked="checked"{/if}>
												<label for="label_{$attr.id_product_attribute}" class="label_label">
													<span class="label_name label_row">{$attr.group_name} {$attr.attribute_name}</span>
													<span class="label_weight label_row">Вес {$attr.weight + $product.weight}</span>
													<span class="label_price">{if !$priceDisplay}{convertPrice price=$product.price + $attr.price}{/if}</span>
												</label>
											</div>
						        {/foreach}
						    {/if}
						...
						{if ($product.id_product_attribute == 0 || (isset($add_prod_display) && ($add_prod_display == 1))) && $product.available_for_order && !isset($restricted_country_mode) && $product.customizable != 2 && !$PS_CATALOG_MODE}
							{if (!isset($product.customization_required) || !$product.customization_required) && ($product.allow_oosp || $product.quantity > 0)}
								...
								<a class="button ajax_add_to_cart_button btn btn-default" href="{$link->getPageLink('cart', true, NULL, $smarty.capture.default, false)|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Add to cart'}" data-id-product-attribute="{$product.id_product_attribute|intval}" data-id-product="{$product.id_product|intval}" data-minimal_quantity="{if isset($product.product_attribute_minimal_quantity) && $product.product_attribute_minimal_quantity >= 1}{$product.product_attribute_minimal_quantity|intval}{else}{$product.minimal_quantity|intval}{/if}">
									<span class="btn_price"><span></span></span><span class="btn_name">{l s='Add to cart'}</span>
								</a>
							{else}
