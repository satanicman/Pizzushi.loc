		...
		<div class="table-responsive-row clearfix">
			<table id="table-combinations-list" class="table configuration">
				...
				<tr class="nodrag nodrop">
					...
					<th class=" left">
						<span class="title_box">Цена</span>
					</th>
					<th class=" left">
						<span class="title_box">Максимальное кол-во</span>
					</th>
					<th class=" left">
						<span class="title_box">Кол-во</span>
					</th>
				</tr>
				</thead>
				<tbody>
				{foreach from=$options item=option name=option}
					<tr>
						<td class=" left">
							<input type="checkbox" name="option[{$smarty.foreach.option.index}][id]" id="option" value="{$option.id_product_option}"{if $option.selected} checked="checked"{/if}>
						</td>
						<td class=" left">
							{$option.name}
						</td>
						...
						<td class=" left">
							<input type="text" name="option[{$smarty.foreach.option.index}][amount]" id="amount" class="amount" data-max_amount="{$option.max_amount}" value="{$option.amount}" style="width: 100%;">
						</td>
					</tr>
				{/foreach}
				</tbody>
		...
		<script>
			$(document).ready(function () {
				$('.amount').keyup(function(event) {
					if($(this).val() > $(this).data('max_amount')){
						$(this).val($(this).data('max_amount'));
					}
					else if($(this).val() < 0){
						$(this).val(0);
					}
