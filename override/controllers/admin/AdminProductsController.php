...
class AdminProductsControllerCore extends AdminController
{
    ...
    public function postProcess()
    {
        ...
        if(!empty($_POST) && !empty($this->object->id)){
            Db::getInstance()->execute("DELETE FROM "._DB_PREFIX_."product_option_product WHERE id_product={$this->object->id}");
            if(isset($_POST['option']))
                foreach ($_POST['option'] as $id_producr_option)
                    Db::getInstance()->execute("INSERT INTO "._DB_PREFIX_."product_option_product SET id_product={$this->object->id}, id_product_option=".$id_producr_option['id'].", amount=".$id_producr_option['amount']);
        }
    }
